...
def regenerate_grub_cfg() -> None:
    ...
    L = [
        ...
        '# via net_bootp.  EFI grub sets its own prefix.  Only override if',
        '# net_default_server was not set (direct PXE without embed.cfg).',
        'if [ -z "$net_default_server" ]; then set prefix=(pxe)/grub; fi',
        '',
...
def write_dnsmasq_conf(iface: str = '') -> str:
    ...
    # pxe-service basenames — PXE client appends .0 (BIOS) or .efi (EFI)
    grub_bios_base = 'grub/i386-pc/core'
    ...
    conf = '\n'.join([
        ...
        f'dhcp-boot={grub_bios_bin},,{ip}',
        '',
        '# PXE service discovery (required for ProxyDHCP responses)',
        '# pxe-service populates option 43/60 vendor extensions that PXE clients',
        '# need to complete the boot server discovery handshake.',
        '# Suffix rules (dnsmasq 2.76+): basenames without a file extension',
        '# get .0 appended; basenames WITH an extension are sent as-is.',
        f'pxe-service=x86PC,"Network Boot",{grub_bios_base},{ip}',
        f'pxe-service=x86-64_EFI,"Network Boot",shimx64.efi,{ip}',
        f'pxe-service=IA32_EFI,"Network Boot",shimia32.efi,{ip}',
        '',
        '# Architecture-specific dhcp-boot overrides (EFI clients)',
...
WEB_PORT = 5656  # default; overridden by --port

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='NetVentoy — PXE boot server')
    parser.add_argument('--port', '-p', type=int, default=5656,
                        help='Web UI / HTTP port (default: 5656)')
